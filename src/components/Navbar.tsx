...
import { getSectionFromPath, scrollToSectionById } from '../utils/router';
...
const navItems: NavItem[] = [
  ...
  { label: 'Medha Exam', href: '/medha-exam', sectionId: 'medha-exam', badge: 'Academy' },
...
export const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    // 1. Initial route check on mount
    const currentPath = window.location.pathname;
    const initialSection = getSectionFromPath(currentPath);
    setActiveSection(initialSection);

    if (currentPath !== '/' && currentPath !== '') {
      // Scroll to the targeted section on direct URL load
      const timer = setTimeout(() => {
        scrollToSectionById(initialSection, true);
      }, 150);
  ...
  useEffect(() => {
    ...
    const handlePopState = () => {
      ...
      scrollToSectionById(section, true);
    };
    window.addEventListener('popstate', handlePopState);

    // 3. Scroll spy listener: sync active section and URL
    const handleScroll = () => {
      ...
      // Active section spy
      const sections = ['home', 'about', 'skills', 'portfolio', 'medha-exam', 'contact'] as const;
        ...
        if (el) {
          ...
          if (scrollPosition >= top && scrollPosition < top + height) {
            ...
            if (window.location.pathname !== targetPath) {
              window.history.replaceState({ sectionId: section }, '', targetPath);
            }
            break;
          }
        }
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {item.badge && (
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-blue-100 text-blue-800 dark:bg-blue-950 dark:text-blue-200 font-medium">
                      {item.badge}
                    </span>
                  )}
                </a>
